	...
	<c:choose>
		...
		<c:otherwise>
			<tr style="${ui_field[type]=='hidden' ? 'display: none;' : ''}">
				...
				<td><span class="fields" name="${info.key}"><src:mk_field name="${info.key }" values="${rcd_value}" type="${ui_field[type] }" link="${ui_field[link] }" link_type="${ui_field[link_type] }" valid="${ui_field[valid] }"  keyValid="${ui_field[keyValid] }" maxlength="${ui_field[maxlength] }" label="${ui_field[label] }"/></span></td>
			</tr>
		</c:otherwise>
	</c:choose>
</c:forEach>
